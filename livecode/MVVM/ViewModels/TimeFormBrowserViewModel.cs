...
{
    public class TimeFormBrowserViewModel
    {
        private Observable<int> _selectedIndex = new Observable<int>();
        private ObservableList<TimeFormViewModel> _items = new ObservableList<TimeFormViewModel>();

        public IEnumerable<TimeFormViewModel> Items => _items;
        ...
        {
            SubmitFormData.SaveTimeForm(SelectedItem.SerializeForm());
        ...
        public TimeFormViewModel SelectedItem => _selectedIndex < 0 ? null : _items[_selectedIndex];


        public TimeFormBrowserViewModel(IEnumerable<TimeFormViewModel> forms)
        {
            _items.AddRange(forms);
            _selectedIndex.Value = 0;
        }

        public void AddToItems(TimeFormViewModel[] newSet)
        {
